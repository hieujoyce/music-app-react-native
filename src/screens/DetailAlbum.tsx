...
import {getDeialAlbum} from '../redux/detailAlbum.slice';

const DetailAlbum = ({route, navigation}: {route: any; navigation: any}) => {
  const [dataDetailAlbum, setDataDetailAlbum] = useState<IAlbum | null>(null);
  const dispatch = useAppDispatch();
  const {albumId} = route.params;
  const {detailAlbum, user, musicData} = useSelector(
    (state: RootState) => state,
  );
  useEffect(() => {
    const findIndex = detailAlbum.findIndex(el => el._id === albumId);
    if (findIndex !== -1) {
      setDataDetailAlbum(detailAlbum[findIndex]);
    } else {
      dispatch(
        getDeialAlbum({
          id: albumId,
          assessToken: user.accessToken,
          artistArr: musicData.artists.data,
        }),
      );
    }
  }, [detailAlbum.length]);
      ...
      {dataDetailAlbum && (
        <View style={{paddingHorizontal: 20}}>
          <View style={{alignItems: 'center'}}>
            <Image
              source={{uri: dataDetailAlbum.image}}
          ...
          <Text
            ...
            {shortstring(dataDetailAlbum.title, 20)}
          </Text>
          <Text style={{color: COLOR.gray, alignSelf: 'center', marginTop: 8}}>
            {1}
            {' Albums  |  '}
            {dataDetailAlbum.songsAmount}
            {' Songs  |  '}
            {toHHMMSS(dataDetailAlbum.time as number)}
            {' mins'}
          </Text>
          <View
            style={{
          ...
          <ScrollView style={{height: 280}}>
            {(dataDetailAlbum.songsAlbum as any).map((el: any, i: number) => {
              return (
                <TouchableOpacity
                  onPress={() => {
